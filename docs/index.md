...
- **再現対象**: 本リポジトリの `package.json` / `package-lock.json` を優先し、Hardhat 3.11.0、Node.js 22.13.0 以上、Solidity 0.8.24、OpenZeppelin Contracts 5.0.2 の学習用構成として扱う。
- **現行公式確認**: 新規プロジェクトでは Hardhat 3 の最新 minor、Solidity 最新リリース、OpenZeppelin Contracts 5.x、Foundry、The Graph、各 L2 / Explorer / RPC の公式ドキュメントを確認し、教材の固定 version を盲目的に転用しない。
- **L2 / Blob 前提**: Dencun / Pectra / Fusaka・PeerDAS / Blob Parameter Only fork により、blob target / max、手数料、データ可用性の前提は変化する。Day08 の数値は実測と公式情報で更新する。
