...
| `mainnet` | `production-mainnet` | `DEPLOY_MAINNET_RPC_URL` / `DEPLOY_MAINNET_PRIVATE_KEY` | Required reviewers、Prevent self-review、administrator bypass禁止、main branch rule 必須 |
| `optimism` | `production-optimism` | `DEPLOY_OPTIMISM_RPC_URL` / `DEPLOY_OPTIMISM_PRIVATE_KEY` | Required reviewers、Prevent self-review、administrator bypass禁止、main branch rule 必須 |

network を分けることで、Sepolia job が Mainnet RPC / key を取得する状態を避ける。production では UI の確認欄へ `DEPLOY_PRODUCTION` を正確に入力し、さらに自己承認を禁止した Environment approval を通す。Settings > Environments で **Allow administrators to bypass configured protection rules** を無効にし、repository administrator による強制 bypass も禁止する。workflow は GitHub API で branch policy、production reviewer、Prevent self-review を再確認し、不足していれば secrets を step へ渡す前に停止する。administrator bypass 設定は Environment REST API の取得項目に含まれないため、管理者が UI で確認し、変更時に再監査する。
...
4. `production-*` Environment の別 reviewer が network と費用を確認する（Prevent self-review 必須）。管理者は事前に **Allow administrators to bypass configured protection rules** が無効であることも確認する。
...
| 症状 | 原因 | 対処 |
|---|---|---|
| input validation が失敗 | contract 名、JSON、production 確認が不正 | Solidity identifier、JSON 配列、固定確認文字列を確認 |
