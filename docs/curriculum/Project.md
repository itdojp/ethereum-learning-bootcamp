...
そこで本ページでは、Day14 の統合（デプロイ＋DApp 接続）を **小さなプロダクトの完成**として、各Dayの位置づけを整理する。
...
## 1. 何を作るか（完成イメージ）
完成形は「コントラクトをデプロイし、DApp から動かして、記録を残せる」状態だ。
...
1) `.env` に `SEPOLIA_RPC_URL` / `PRIVATE_KEY` を設定し、少額のテスト ETH を入れる
2) デプロイする：
```bash
...
| Day04 | Solidity 基礎 | 状態・イベント・revert の基本形を実装し、テストで検証する練習をする |
...
| Day14 | 統合 | デプロイ→DApp 接続→（任意）Verify/CI/The Graph を通し、成果物として残す |
