...
## 最短完走（この本のゴール）
- [ ] ルートで `npm test` が通る
- [ ] 任意ネットワークへデプロイし、DApp から接続して動作確認できる
- [ ] 記録（`docs/DEPLOYMENTS.md` と `docs/reports/`）を残せる

## Phase 5 実務レビューゲート
- [ ] Hardhat 3のminor version、Node.js、Solidity、OpenZeppelin Contracts、Foundryの公式ドキュメント差分を確認した
- [ ] Dencun / Pectra / Fusaka・PeerDAS / BPO による L2/Blob 前提の変化を確認し、計測時刻・chainId・txHash・RPC を記録した
- [ ] 秘密鍵、リカバリーフレーズ、RPC/API キー、署名リクエスト、approval、bridge 操作を学習用アカウントに限定した
- [ ] PR では GitHub Copilot review の本文・inline comment・suggestion を全件確認し、未解決 review thread 0 を確認した
...
- [ ] Day3：`npm run install:reviewed` → `npm test` が通り、（任意）テストネットへデプロイできる（本文：[`docs/curriculum/Day03_Env_Setup.md`](./Day03_Env_Setup.md)）
...
### Part 5：統合
- [ ] Day14：デプロイ→DApp 接続→（任意）Verify/CI/The Graph を、できる範囲で統合できる（本文：[`docs/curriculum/Day14_Integration.md`](./Day14_Integration.md)）
