...
Hardhat 2.x 互換の `@nomicfoundation/hardhat-verify` 2.1.3 は、`etherscan.apiKey` を文字列にすると Etherscan V2 endpoint と `chainid` を使う。対応ネットワークは秘密情報なしで確認できる。

```bash
npx hardhat verify --list-networks
```

`optimismSepolia` は同版の built-in network にないため、本リポジトリでは V2 endpoint を指定した `customChains` を必要最小限で追加している。Optimism mainnet は built-in chain ID 10 を使う。
