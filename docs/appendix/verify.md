...
## 4. このリポジトリの環境変数
- RPC：
  - `SEPOLIA_RPC_URL`, `OPTIMISM_SEPOLIA_RPC_URL`, `MAINNET_RPC_URL`, `OPTIMISM_RPC_URL`
- API キー：
  - Etherscan API V2（Sepolia / OP Sepolia / Mainnet / Optimism）：`ETHERSCAN_API_KEY` 1つ

本リポジトリは `@nomicfoundation/hardhat-verify` 3.0.21をexact pinし、`verify.etherscan.apiKey` にEtherscan API V2の単一keyを設定する。plugin登録とproviderを秘密情報なしで確認する場合はhelpを使う。
...
Hardhat 3.11.0のchain descriptorでSepolia、OP Sepolia、Mainnet、Optimismを解決する。未収録networkを追加する場合は `chainDescriptors` を使用し、chain IDとexplorer URLを一次資料で確認する。
...
| API key が必要と言われる / 失敗する | API キー未設定 / 空 | `.env` に `ETHERSCAN_API_KEY` が入っているか | Etherscan V2 のキーを入れて再実行（キーはコミットしない） |
...
- その場合、`hardhat.config.ts` の `chainDescriptors` 設定が必要になることがある。
- Etherscan V2 対応外の explorer を追加する場合は、API version、chain ID、plugin compatibility を一次資料で確認し、Etherscan V2 用の単一キー契約と混同しない。
