...
この補足は「AAって何？」「4337と7702はどう違う？」を、初学者向けに最小限で整理します。
...
**ポイント**：チェーンのコンセンサス変更なしで、コントラクト構成だけでAAを実現します。
...
**ポイント**：プロトコル側の変更により、EOAが **delegation indicator（委任先）** をセットして、実行時に別アドレスのコードへ委譲する挙動が入り得ます。
...
## 4. 4337と7702の関係（整理）
- **競合というより“レイヤーが違う”** と捉えると混乱しにくいです。
  - **ERC‑4337**：コントラクトだけでAAを実現（ネットワーク変更不要）
  - **EIP‑7702**：EOAの実行モデルを拡張（ネットワーク側のアップグレードが必要）
- **組み合わせる設計**も考えられます（例：4337のアカウント実装を、7702の委任先コードとして使う等）。

> 注：どのチェーンでどの機能が有効かは時期により変わります。実際の利用可否は各チェーンの公式アナウンス・EIPを確認してください。
